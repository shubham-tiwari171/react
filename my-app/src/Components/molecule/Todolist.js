      ...
      <div className={styles.container}>
        ...
        <ul className="list-group todos mx-auto text-light">
          {todos.map((data) => (
            <li key={data.id} className={`${styles['list-group-item']} d-flex justify-content-between align-items-center`}>
              <div className={`${styles.text}`}><span style={{ marginLeft: '2rem' }}>{data.task}</span></div>
              <button id={data.id} className={`btn ${styles.icon}`}><MdCheckCircle size={25} /></button>
              <div className={`btn ${styles.icon}`}><MdModeEdit size={25} /></div>
              <div className={`btn ${styles.icon}`}><MdDelete size={25} /></div>
            </li>
          ))}
        </ul>
